#!/usr/bin/env bash
# Durable C+ supervisor. Same profile as valid C 20260813r2 plus isolated
# local pickle writes. Never writes to /data/live or production spool.
# Probe receive_mode=immediate_drain_unbounded_app_queue. Unchanged:
...
arm_root="$ROOT/arm_Cplus"
write_dir="$arm_root/writes"

cat >"$marker" <<EOF
DO_NOT_TOUCH: active Track (D) standalone C+ local-write latency experiment
run_id=$RUN_ID
...
"$PYTHON" - "$manifest" "$RUN_ID" "$ROOT" "$CODE_ROOT" "$DURATION_SEC" "$WARMUP_SEC" "$write_dir" "$WRITE_BATCH_SIZE" "$WRITE_INTERVAL_SEC" "$WRITE_QUEUE_MAX" "$(snapshot_background)" <<'PY'
import json
import subprocess
import sys
...
path, run_id, root, code_root, duration, warmup, write_dir, batch, interval, queue_max, background = sys.argv[1:]
...
payload = {
    "run_id": run_id,
    "kind": "standalone_cplus_local_write",
    ...
    "baseline_valid_c": "wsfanout_bc_20260813r2 arm C",
    "controlled_factors": {
        "host": "same NEW VPS as valid C 20260813r2",
        ...
        "duration_sec": int(duration),
        "warmup_sec": int(warmup),
        "telemetry": "raw XRP delivery, raw loop lag, runtime/reconnect/resource metrics, fresh matched XRP ping, write bytes/files/latency/queue/drops",
        "resource_limits": {"max_load_1": 8, "min_available_mib": 4096, "max_rss_mib": 2048, "max_fds": 4000, "max_cpu_percent": 95},
        "background_policy": "recorded before/after; not toggled by this series",
        "bars": False,
        "spool": False,
        "remote_backup": False,
        "compaction_changes": False,
    },
    "sole_differing_factor": "C+ is valid C plus isolated local pickle batch writes of processed records; no pandas/pyarrow on the tick path.",
    "write_policy": {
        "format": "pickle",
        "why": "Lowest extra CPU/GIL/syscall cost vs parquet: pickle.dumps of compact tuples on a worker thread; no DataFrame/Table construction, no pyarrow, no per-row fsync.",
        "dir": write_dir,
    ...
    "validity_gates": {
        ...
        "safety_abort": 0,
        "failure_verdict": "measurement_failed; no causal inference vs C 20260813r2 if gates fail",
    },
    ...
    "artifacts": {
        "runtime": "arm_Cplus/runtime.jsonl",
        "ping": "arm_Cplus/ping_xrp.log",
        "raw_delivery": ["arm_Cplus/xrp_delivery_okx.csv", "arm_Cplus/xrp_delivery_bybit.csv"],
        "loop_lag": "arm_Cplus/loop_lag.csv",
        "write_metrics": "arm_Cplus/write_metrics.csv",
        "write_flush": "arm_Cplus/writes/write_flush.csv",
        "write_dir": write_dir,
    },
}
...
preflight
mkdir -p "$arm_root" "$write_dir"
arm_start="$(date -u +%FT%TZ)"
background_before_arm="$(snapshot_background)"
"$PYTHON" - "$manifest" "$arm_root/arm_manifest.json" "$arm_start" "$background_before_arm" <<'PY'
import json
import sys
from pathlib import Path

run, target, start, background = sys.argv[1:]
...
Path(target).write_text(json.dumps({
    ...
    "expected": {
        ...
        "write": source["write_policy"],
    },
    "controlled_factors": source["controlled_factors"],
...
PY
printf '%s supervisor_start run_id=%s arm=C+ exact_end_utc=%s\n' \
  "$arm_start" "$RUN_ID" "$("$PYTHON" -c "import json; print(json.load(open('$manifest'))['exact_end_utc'])")" | tee -a "$status"
...
"$PYTHON" "$CODE_ROOT/ws_fanout_three_arm.py" \
  --arm C+ --run-id "$RUN_ID" --duration-sec "$DURATION_SEC" \
...
payload.update({
    ...
    "ping_exit_status": int(ping_status),
