...
Скопировать в **новый** чат. Это **экспериментальный** контур модели (гипотезы и falsifiable checks **вокруг закрытого гира 2.2** и следующих ступенек).
...
Цель: проектировать и прогонять фальсифицируемые эксперименты **вокруг
закрытого гира 2.2** (наблюдение / упрощённый прогон: `research/gear22_backtest/`,
1 Гц, `K=1`, `SLOT_MODE=global`, замороженные ручки в `params_frozen.py`; заполнение =
`spread_last`, не `Trade_Lat`). Следующий шаг наблюдения — сравнить счётчики
с `would_send`, затем с `Trade_Lat`. Идеи 2.5 (политика размера) и 3 (поиск VARIATION)
...
Close 2.2 как наблюдения ≠ готовность к живой торговле.
...
- Гир 2.2 = закрыт (наблюдение; упрощённый прогон / замороженные ручки).
  Канон: docs/strategy-gears.md. C/D не критерий этого close. 2.5 и 3 —
  blocked до явного unlock (этот close не unlock).
...
- [ ] Назвать один фокус наблюдения вокруг закрытого 2.2 (`would_send` / `Trade_Lat`, не 2.5/3).
