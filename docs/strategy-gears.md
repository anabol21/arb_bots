...
| Сигнальный фундамент / **гир 0.8** | Закоммиченный снимок: сигнальный прогон + гейты (+ задел исполнения); **не** закрытый гир 1 |
| Закрытый гир 1 (**1.0**) | Сигналы + слой исполнения + **зафиксированный** вектор + контракт боевого контура; после валидации, явных приближений vs боевой бот и методологии параметров фильтров |
...
Сигнальный фундамент и закоммиченный baseline в репозитории — это **гир 0.8**, а **не** закрытый гир 1 (**1.0**).

До закрытия гира 1.0 явно остаются: **валидация**; **явный учёт приближений**, из‑за которых стратегия может отличаться от реального боевого бота; **методология определения параметров фильтров**.
...
### Что ещё нужно, чтобы закрыть гир 1 (выйти из 0.8 в 1.0)

1. **Валидация** прогона и инвариантов на реалистичных данных/сценариях.
2. **Явный учёт приближений** симуляции vs боевой бот (и где расхождение допустимо).
3. **Методология определения параметров фильтров** (как выбираются и замораживаются гейты/пороги).
4. Выбрать и зафиксировать **боевой профиль** исполнения (какие ручки включены и с какими значениями).
5. Зафиксировать **один боевой вектор** вариации и набор гиперпараметров.
6. Оформить **контракт встраивания в боевой контур:** совпадают пороги, доли усреднения, гейты A/B и обязательные ручки исполнения; расхождения допустимы только в фактическом исполнении на бирже — и перечислены явно.
7. Санитарные прогоны на нескольких днях (при необходимости — на нескольких монетах как проверка кода), без заявления о торговом преимуществе.
...
Сейчас репозиторий на **гире 0.8**. Путь: **закрыть гир 1 (1.0) → порог гира 2**. Прыжок к гиру 3 запрещён. Гир 2.5 — после осмысленного гира 2.

1. Закрыть разрыв 0.8→1.0: валидация, явные приближения vs боевой бот, методология параметров фильтров; затем минимальный профиль исполнения и перечень упрощений симуляции.
