...
**Стало:** целевые спреды редки и режимны → сначала скринер режима по рынку (**1.5**), затем мультимонетная торговля **фиксированной** моделью 1.0 (**2**), затем **наблюдение / упрощённый прогон** (**2.2**, закрыт), затем политика размера (**2.5**, заблокирован до unlock), и только потом поиск в пространстве параметров по каталогу аномальных эпизодов (**3**). Бары `5m` нужны с **гира 1.5**.
...
|-----|-------|---------|-------|---------|---------|-------|
| Охват | Одна монета, фиксированные правила | Скринер режима по доступному рынку | Мультимонета: long/short любыми монетами | Тот же охват 2 + таблица признаков 1 Гц и упрощённый прогон | Мультимонета + политика размера | Эпизоды аномалий; поиск параметров |
| Адаптивность | Вектор зафиксирован | Пороги скринера **экспертные**, фиксированные | Тот же вектор 1.0; v0 = все crypto; кластер 1.5 — open-only флаг | Замороженные ручки наблюдения; без поиска `VARIATION`; без доли позиции | Политика `position_frac` / размера | Поиск `VARIATION` (+ при нужде порогов скринера) |
| Данные и риск подгонки | Мало степеней свободы | Риск ложных «горячих» меток | Риск капитала и корреляций без обучения порогов | Риск принять метку наблюдения за прибыль | Подгонка размера под хвосты | Подгонка порогов — только при пуле эпизодов |
| Операционная сложность | Симулятор на истории | Бары `5m` + mid; Top‑N / кластер | Лимит `K` слотов, равный капитал на слот | Улей признаков, `decide()`, свип ручек | Отчёт риска размера | Train/test по эпизодам |
...
flowchart TB
  ...
  g22["Гир 2.2 наблюдение / замороженные ручки"]
...
Гир **2.2 закрыт как наблюдение** (см. ниже). Это **не** разблокировка гира 2.5 и **не** старт гира 3. Вход в гир 2.5 — только по явному unlock: мультимонета + **доступный** флаг кластера 1.5 + константный равный слот. Не читать close контура 2 как «кластер — вселенная по умолчанию»; C/D не сделаны в гире 2 и **не** являются критерием close гира 2.2.

---

## Гир 2.2 — наблюдение / упрощённый прогон (**закрыт**)

### Смысл

Именованная ступень лестницы **после** закрытого контура гира 2: таблица признаков 1 Гц, упрощённая политика и замороженные ручки наблюдения на том же мультимонетном охвате. **Не** гейт симулятора гира 1.0, **не** политика размера, **не** поиск `VARIATION`.

**Статус (2026-09-14):** гир 2.2 = закрыт (наблюдение; упрощённый прогон / замороженные ручки). Штамп без скобок запрещён. Закрыто: канон флора, `state p50`, таблица признаков, `decide()` / прогон / свип, кандидат ручек в [`params_frozen.py`](../research/gear22_backtest/params_frozen.py). Доказательство = **гребень** на сетке, **не** плато 20–30%, **не прибыль**. `combined_mark` — метка наблюдения, не PnL. Чтение будущего **не доказано**. Заполнение — `spread_last` той секунды (1 Гц), не `Trade_Lat`. Не закрыто и не утверждается: готовность к живой торговле; ветки C/D как критерий закрытия; сравнение с журналом `would_send`; заполнение `Trade_Lat`. Проверка этого close — локальные модульные тесты политики / прогона / свипа, не VPS и не живой контур.

### Замороженные ручки наблюдения

Канон: [`gear22-observe-knob-sweep.md`](gear22-observe-knob-sweep.md), код [`research/gear22_backtest/params_frozen.py`](../research/gear22_backtest/params_frozen.py) (`FROZEN` = `DEFAULT_OBSERVE_PARAMS`). `SLOT_MODE=global` (`K=1` на все монеты).
...
| `p50_open` | `0.60` |
| `min_profit_pp` | `0.20` |
| `min_theta_close` | `0.05` (гейт **включён**; `0` ≠ `None`) |
| `min_spread_open` | `None` (гейт выключен) |
| `fee_round_trip_pp` | `0.30` |

`None` на необязательном гейте его выключает. `min_theta_close=0` включает строгий порог «больше» и отказ при NaN. На гребне лежит `0.05`, не `None`.

### Что входит (этот close)

- Наблюдательный **floor** (не гейт симулятора): [`gear22-floor-metric.md`](gear22-floor-metric.md) — `tf-select α25` trim-средних SMA-12 за 3h/12h. Не классификатор режима и не порог входа.
- Наблюдательные **state p50** (не гейт симулятора): [`gear22-state-p50.md`](gear22-state-p50.md) — `p50_bar5m` / `p50_roll_1m` / `p50_roll_5m` (TW hold→next). Не порог входа.
- Наблюдательная **таблица признаков 1 Гц** (не гейт симулятора): [`gear22-backtest-features.md`](gear22-backtest-features.md) — широкая строка `(coin, ts_s)`, улей `coin=/event_date=`. Продюсер: [`research/gear22_feature_day_probe.py`](../research/gear22_feature_day_probe.py).
- Упрощённая политика и прогон: [`research/gear22_backtest/`](../research/gear22_backtest/) (`policy.py`, `replay.py`, `sweep.py`, `replay.ipynb`).
- Свип ручек: нет плато 20–30% (0 из 192 на дешёвой сетке); найден гребень; консервативный кандидат зафиксирован. `combined_mark` — сумма закрытых `potential_pp` плюс оценка ещё открытой позиции, не прибыль.

### Вне гира 2.2 / вне этого close

- Политика доли позиции / размера (**гир 2.5**) — по-прежнему заблокирована до явного unlock. Этот close **не** разблокирует 2.5.
- Поиск `VARIATION` / оптимизация порогов (**гир 3**).
- Гейт симулятора гира 1.0 (`Trade_Lat`, очередь fill, `run_backtest`).
- Сравнение прогона 1 Гц с журналом `would_send`, затем с `Trade_Lat` — **следующий шаг наблюдения**, не этот close.
- Ветки C (`regime_on`) и D (случайный вход) как критерий закрытия — не сделаны здесь.
- Смена замороженных `HYPER` 1.0 без явного unlock.
- Утверждение торгового преимущества или готовности живой отправки заявок. Журнал `would_send` (если он уже есть в треке склейки) **не** входит в этот close.

### Вход

Закрытый гир 2 = закрыт (контур; 2.2 вне scope). Канон счётчиков гира 2: [`gear-2-close-20260825.md`](gear-2-close-20260825.md). Покрытие данных: [`model-data-coverage.md`](model-data-coverage.md).
...
Закрытый контур гира 2 и **явный unlock** гира 2.5: мультимонета + **доступный** флаг кластера 1.5 + константный размер. Кластер не считается включённой вселенной по умолчанию. Признаки режима (бары, mid) уже есть из 1.5. Гир 2.2 закрыт как наблюдение; этот close **не** разблокирует политику размера. Старт 2.5 **запрещён** до явного unlock.
...
| 2.2 | **закрыт (наблюдение; упрощённый прогон / замороженные ручки):** [`params_frozen.py`](../research/gear22_backtest/params_frozen.py), [`gear22-observe-knob-sweep.md`](gear22-observe-knob-sweep.md); флор / p50 / таблица признаков | — | Политика размера (2.5); поиск `VARIATION` (3); прибыль; готовность к живой торговле; `would_send` / `Trade_Lat` как этот close |
| 2.5 | Константный размер в гире 2; **заблокирован** до явного unlock (close 2.2 как наблюдения недостаточен) | Политика размера; отчёт риска; тест не смешан с подгонкой | Поиск порогов сделки; отключение исполнения; старт без unlock |
...
В треке модели **гиры 1.0 и 1.5 закрыты**; **гир 2 = закрыт (контур; 2.2 вне scope)**; **гир 2.2 = закрыт (наблюдение; упрощённый прогон / замороженные ручки)**. Гир 1.5 — скринер более волатильных монет (Top‑N / кластер, blend `α≈0.75`), не булев гейт `regime_on`. Прыжок к **2.5** или поиску параметров (**гир 3**) запрещён. Живой бот — трек склейки; монтирование ручек 2.2 в отправку заявок — не сейчас.

1. **Наблюдение (не новый гир):** повторить прогон на замороженных ручках; сравнить счётчики с журналом `would_send` на тех же снимках 1 Гц; затем сравнить заполнение 1 Гц с `Trade_Lat`. Не 2.5 и не 3.
2. Гир 2.5 (политика размера) — только после явного unlock. Close гира 2.2 как наблюдения **не** является unlock.
