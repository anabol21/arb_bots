...
Детектор **аномального режима** (волатильность mid, объём свечи `5m`, перцентили/отношение к медиане) по доступным монетам на истории. Выход — флаг или ранг режима на монету/окно. Пороги скринера **экспертные и фиксированные**: гир 1.5 **не** оптимизирует PnL и **не** подбирает пороги сделок гира 1.

### Вход

Закрытый гир 1.0. Запрос данных: слой баров `5m` (`volume`) + mid/амплитуда офлайн из L1; см. [`docs/data-format-model.md`](data-format-model.md).

### Что входит

- Воспроизводимый расчёт признаков режима по рынку (история, симуляция).
- Фиксированные пороги/перцентили скринера.
- Санитария: на тихом рынке почти нет «горячих» меток; на хвостах метки осмысленны и стабильны при тех же правилах.
...
| 1.5 | Запрос баров `5m` в docs | Воспроизводимый скринер режима; фиксированные пороги; санитария тихий/хвост | Max-PnL подбор порогов скринера; торговля без флага |
...
В треке модели **гир 1.0 закрыт**. Следующий порог — **гир 1.5**. Прыжок к поиску параметров (гир 3) запрещён. Живой бот — трек склейки.

1. Спека и каркас гира 1.5 (бары + mid → флаг режима); handoff сборщику по слою `bar_5m`.
