...
| Адаптивность | Вектор зафиксирован | Пороги скринера **экспертные**, фиксированные | Тот же вектор 1.0; возможности **в кластере** 1.5 | Политика `position_frac` / размера | Поиск `VARIATION` (+ при нужде порогов скринера) |
...
| Операционная сложность | Симулятор на истории | Бары `5m` + mid; Top‑N / кластер | Лимит `K` слотов, равный капитал на слот | Отчёт риска размера | Train/test по эпизодам |
...
Гир 2 ищет сделки модели 1.0 **в кластере** скринера 1.5, а не по всему тихому рынку. Булев `regime_on` как жёсткий гейт входа — **опция позже** (реализация гира 2), не критерий закрытия 1.5.
...
Применение **чёрного ящика гира 1.0** (замороженные `VARIATION` и `HYPER`) по **доступному рынку**: long/short по монетам с полным L1 из **кластера** скринера 1.5. Доля позиции — **константа** профиля (не обучаемая политика).

### Вход

Закрытый гир 1.0 и закрытый гир 1.5 (Top‑N / кластер волатильных монет). Периоды — ряды **полного** L1; legacy без book не смешивать без явной пометки.
...
- Вход в кластере скринера 1.5 (жёсткий `regime_on` — опция реализации, не долг 1.5).
...
Закрытый гир 2: мультимонета + кластер 1.5 + константный размер. Признаки режима (бары, mid) уже есть из 1.5.
...
| 2 | Чёрный ящик 1.0; закрытый скринер 1.5 | Мультимонетный симулятор; вход в кластере 1.5; лимит `K` и равный слот; отчёт | Поиск `VARIATION`; политика размера |
...
| 3 | — | Поиск по эпизодам; train/test; отчёт устойчивости | Старт без 2+2.5; «победитель» без теста |

---

## Ближайший порядок работ

В треке модели **гиры 1.0 и 1.5 закрыты**. Гир 1.5 — скринер более волатильных монет (Top‑N / кластер, blend `α≈0.75`), не булев гейт `regime_on`. Прыжок к поиску параметров (гир 3) запрещён. Живой бот — трек склейки.

1. Каркас гира 2: мультимонета вокруг чёрного ящика 1.0 в кластере скринера 1.5; `K` слотов и равный капитал.
2. Handoff сборщику по слою `bar_5m` (данные, не долг закрытия 1.5).
3. После закрытия 2 — гир 2.5 (политика размера).
4. Гир 3 — только при пуле аномальных эпизодов и закрытых 2–2.5.
