...
## Контур Hyperliquid L1

Отдельный набор, не тело lean и не v1. Корень `/data/live-hl`, партиция `base_coin=<COIN>/event_date=<YYYY-MM-DD>/`. Колонки и запрет спреда: [`hl-l1-storage-contract.md`](hl-l1-storage-contract.md). Этот документ не добавляет колонки Hyperliquid в parquet Bybit/OKX.
...
| **hl_l1** | отдельный процесс `python -m app.hl` | корень `/data/live-hl`; не колонки lean/v1; контракт [`hl-l1-storage-contract.md`](hl-l1-storage-contract.md) |
