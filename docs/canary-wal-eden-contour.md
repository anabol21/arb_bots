...
OKX/Bybit orders/positions subscribe the **same** runtime coin
...
while the profile coins are WAL/EDEN is rejected. OKX `fills` is VIP/fee-tier
gated (`64003`) and is not subscribed; chronometry reads `fillPx` / `avgPx`
/ `uTime` from the `orders` channel.
