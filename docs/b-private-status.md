...
| Warm-Lat | Замер стадий на **уже тёплой** private+trade сессии; A/B (queue→send vs W6/W7 prepare); dry `send=false` + gated live | Пакет в коде; live на VPS — человеком, tiny notional |
...
- Warm-Lat: `--ws-warm-latency --warm-lat-n=N [--warm-lat-path=AB] [--warm-lat-send=false]` (dry); live: `BBOT_PRIVATE_WARM_LAT=1` + `--warm-lat-send=true --warm-lat-approve-one-shot` + `VENUE=live` + `LIVE_ORDERS=1` — см. [`b-private-warm-latency-experiments.md`](b-private-warm-latency-experiments.md)
