...
class TestDropDelistedOpen(unittest.TestCase):
    def test_drops_terminal_icx_and_keeps_other_open(self) -> None:
        from research.gear22_backtest.sweep import RunResult
        from research.gear22_backtest.venue_balance_run import drop_terminal_delisted_open
        ...
        dropped = drop_terminal_delisted_open(icx)
        self.assertIsNone(dropped.open_coin)
        self.assertIsNone(dropped.open_ts)
        self.assertEqual(dropped.mtm_pp, 0.0)
        self.assertEqual(dropped.exposure_s, 250)
        ...
        self.assertIs(drop_terminal_delisted_open(other), other)


class TestCoinNet(unittest.TestCase):
    def test_round_net_is_price_pnl_minus_fees(self) -> None:
        from research.gear22_backtest.venue_coin_pnl import coin_totals, round_net_usd

        self.assertAlmostEqual(round_net_usd(3.0, 1.0, 0.2, 0.1), 3.7)
        ranked = coin_totals(
            [
                {"coin": "ICX", "net": 2.0, "pnl": 2.5, "fees": 0.5},
                {"coin": "LA", "net": 1.0, "pnl": 1.2, "fees": 0.2},
                {"coin": "ICX", "net": -0.5, "pnl": 0.0, "fees": 0.5},
            ]
        )
        self.assertEqual([row["coin"] for row in ranked], ["ICX", "LA"])
        self.assertEqual(ranked[0]["n"], 2)
        self.assertEqual(ranked[0]["wins"], 1)
        self.assertAlmostEqual(ranked[0]["net"], 1.5)
        self.assertAlmostEqual(ranked[0]["share"], 1.5 / 2.5)
