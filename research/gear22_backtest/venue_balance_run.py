...
# Terminal open only. Closed rounds on these coins stay in the list.
# ICX was delisted during the still-open leg; a future universe filter is separate.
DELISTED_OPEN_COINS = frozenset({"ICX"})
...
def drop_terminal_delisted_open(result: RunResult) -> RunResult:
    """Drop a still-open leg whose coin was delisted. Closed rounds stay."""
    if result.open_coin is None or str(result.open_coin).upper() not in DELISTED_OPEN_COINS:
        return result
    held = 0
    if result.open_ts is not None and result.mtm_ts is not None:
        held = max(0, int(result.mtm_ts) - int(result.open_ts))
    return replace(
        result,
        open_coin=None,
        open_side=None,
        ...
        mtm_pp=0.0,
        mtm_ts=None,
        exposure_s=max(0, int(result.exposure_s) - held),
    )
...
def run_window(
    *,
    ...
    for label, params in labelled:
        ...
        result = drop_terminal_delisted_open(run_combo(store, params))
    ...
    if window == "all" and runs:
        # Same frozen gates, coin pool without ICX from the first second.
        # Only method 3: equalize after each close, notional = (Bybit + OKX) / 2.
        import gc

        pool = [c for c in store.coins if str(c).upper() not in DELISTED_OPEN_COINS]
        print(
            f"reloading hive without {','.join(sorted(DELISTED_OPEN_COINS))} "
            f"coins={len(pool)}",
            flush=True,
        )
        del store
        gc.collect()
        store_no = store_from_hive(hive, dates=dates, coins=pool)
        print(
            f"  rows={store_no.n_rows} coins={len(store_no.coins)} span_s={store_no.span_s}",
            flush=True,
        )
        params = FROZEN
        result = drop_terminal_delisted_open(run_combo(store_no, params))
        fill_pp, exit_pp = open_fill_exit_pp(store_no, result)
        keys = timestamp_keys_for_run(result)
        print(
            f"  run_combo no_icx closed={result.n_closed} "
            f"open_end={0 if result.open_coin is None else 1} "
            f"open={result.open_coin} keys={len(keys)}",
            flush=True,
        )
        extra_books = (
            load_last_books(tick_dir, keys, books_1hz_dir=sidecar) if keys else {}
        )
        print(f"  no_icx books found={len(extra_books)} / {len(keys)}", flush=True)
        mode: Mode = "balance_equalize"
        ledger = _ledger_for_run(
            result, mode, extra_books, open_fill_pp=fill_pp, open_exit_pp=exit_pp
        )
        status = ledger_status(ledger)
        summary_rows.append(
            summary_row(
        ...
        for step in ledger.steps:
            step_rows.append(step_row("no_icx", "frozen", mode, step))
        charts.append(("no_icx frozen balance_equalize", cash_path(ledger)))
        print(
            f"  no_icx {mode}: status={status} "
            f"final=({ledger.cash_bybit:.4f},{ledger.cash_okx:.4f}) "
            f"min=({ledger.min_bybit:.4f},{ledger.min_okx:.4f}) "
            f"applied={ledger.n_applied} hole={ledger.n_skipped_hole} "
            f"mismatch={ledger.n_price_mismatch} open_end={ledger.n_open_end}",
            flush=True,
        )
