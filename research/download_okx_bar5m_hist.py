"""Download OKX SWAP 5m history (OHLC + volCcy) for vacation-dump coins.

Writes hive-ish layout next to the vacation dump for regime-metric calibration:

  output/okx_bar5m_hist_regime/
...
volume = OKX candle field volCcy (base coin), same canon as live bar_5m.
...
Example:
  python3 research/download_okx_bar5m_hist.py
  python3 research/download_okx_bar5m_hist.py --start 2026-07-08 --end 2026-08-08 --workers 4
  python3 research/download_okx_bar5m_hist.py --limit-coins 10
"""
...
DEFAULT_OUT = REPO / "output" / "okx_bar5m_hist_regime"
...
def list_dump_coins(tick_dir: Path) -> list[str]:
    ...
    # One window already has the full universe for that run.
    table = pq.read_table(files[len(files) // 2], columns=["base_coin"])
    coins = sorted(set(table.column(0).to_pylist()))
    return coins


def load_okx_map(universe_csv: Path, coins: Iterable[str]) -> dict[str, str]:
    ...
    return dict(zip(sub["base_coin"], sub["okx_symbol"]))


def okx_get(params: dict, *, timeout: float = 30.0, retries: int = 5) -> list:
    qs = urllib.parse.urlencode(params)
    url = f"{OKX_HISTORY}?{qs}"
    ...
    raise RuntimeError(f"OKX request failed after retries: {last_err}")
...
def fetch_candles(
    ...
    *,
    ...
    """Fetch [start_ms, end_ms) candles newest-page pagination via `after`."""
    out: list[list[str]] = []
    after: Optional[str] = str(end_ms)
    guard = 0
    while guard < 10_000:
        guard += 1
        params = {
...
def download_one(
    ...
    *,
    ...
    for range_start, range_end in coalesce_ranges(missing):
        ...
        rows = fetch_candles(inst_id, start_ms, end_ms, sleep_s=sleep_s)
...
def main() -> None:
    ...
    ap.add_argument("--start", type=str, default=DEFAULT_START, help="UTC start date inclusive")
    ap.add_argument("--end", type=str, default=DEFAULT_END, help="UTC end date exclusive")
    ap.add_argument("--workers", type=int, default=3, help="parallel coins (keep low for rate limits)")
    ap.add_argument("--sleep", type=float, default=0.08, help="sleep between OKX pages per worker")
    ap.add_argument("--limit-coins", type=int, default=0, help="debug: only first N coins")
    ap.add_argument(
        "--coins",
        type=str,
        default="",
    ...
    mapping = load_okx_map(args.universe, coins)
    ...
    meta = {
        ...
        "source": "OKX /api/v5/market/history-candles bar=5m volume=volCcy",
        "purpose": "regime metric calibration (gear 1.5)",
    }
    ...
    with ThreadPoolExecutor(max_workers=max(1, args.workers)) as pool:
        ...
        for fut in as_completed(futs):
            ...
            except Exception as exc:  # noqa: BLE001 — collect per-coin failures
