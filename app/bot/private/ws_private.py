...
class PrivateStreamRuntime:
    ...
    def _parse_okx(self, data: Mapping[str, Any]) -> ParsedStreamEvent:
        ...
        if event == "error":
            LOG.warning(
                "ws_okx_private_error code=%s has_arg=%s authenticated=%s",
                sanitize_venue_code(data.get("code")) or "unknown",
                isinstance(data.get("arg"), Mapping) and bool(data.get("arg")),
                self.authenticated,
            )
