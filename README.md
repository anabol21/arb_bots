...
Дорожная карта гиров (кратко): `0.8` → `1.0` (закрыт) → `1.5` (закрыт) → `2` (мультимонета, фиксированная модель 1.0) → `2.5` (политика размера) → `3` (поиск параметров по эпизодам аномалий). Подробности: [`docs/strategy-gears.md`](docs/strategy-gears.md).
...
Пока только планы: связать логику модели со скриптом данных, приватные каналы, сделки, портфель. Как приоритет не начат. Закрытие гира `1.5` в симуляторе этот трек не закрывает.
